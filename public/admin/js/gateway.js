...
(function ($) {
    ...
    $('.add-currency').on('click', function (e) {
        ...
        html += '<div class="input-group mb-3 currency-conversation-rate">' +
            '<select name="currency[]" class="form-control currency" required style="background-color: var(--bg-primary, #0B0E11); color: var(--text-primary, #E6EAF0); border: 1px solid var(--border-dark, #1F2630);">';
        Object.entries(allCurrency).forEach((currency) => {
            html += '<option value="' + currency[0] + '">' + currency[1] + '</option>';
        });
        html += '</select>' +
            '<span class="input-group-text" style="background-color: var(--bg-elevated, #171C23); color: var(--gold, #D4AF5A); border: 1px solid var(--border-dark, #1F2630);">1  ' + getCurrencySymbol + ' = </span>' +
            '<input type="number" step="any" min="0" name="conversion_rate[]" value="" class="form-control" required style="background-color: var(--bg-primary, #0B0E11); color: var(--text-primary, #E6EAF0); border: 1px solid var(--border-dark, #1F2630);">' +
            '<input type="hidden" step="any" min="0" name="currency_id[]" value="" class="form-control" required>' +
            '<span class="input-group-text append_currency" style="background-color: var(--bg-elevated, #171C23); color: var(--gold, #D4AF5A); border: 1px solid var(--border-dark, #1F2630);"></span>' +
            '<button type="button" class="border-0 font-24 mr-5 ms-3 removedItem text-danger bg-transparent" title="Remove"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M19 6.41L17.59 5L12 10.59L6.41 5L5 6.41L10.59 12L5 17.59L6.41 19L12 13.41L17.59 19L19 17.59L13.41 12L19 6.41Z"/></svg></button>' +
            '</div>';
    ...
    window.getDataEditRes = function (response) {
        ...
        if (response.data.gateway.slug == 'bank') {
            ...
            if (banks.length > 0) {
                Object.entries(banks).map(function (bank) {
                    ...
                    bankHtml += `<div class="multi-bank bg-off-white radius-4 theme-border pb-0 mb-25">
                                    <div class="row">
                                        <div class="col-6">
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        ...
                        <select name="bank[status][]" class="primary-form-control bank-status" id="status">
        ...
        response.data.currencies.map(function (data) {
            ...
            html += '</select>' +
                '<span class="input-group-text" style="background-color: var(--bg-elevated, #171C23); color: var(--gold, #D4AF5A); border: 1px solid var(--border-dark, #1F2630);">1  ' + getCurrencySymbol + ' = </span>' +
                '<input type="number" step="any" min="0" name="conversion_rate[]" value="' + data.conversion_rate + '" class="form-control" required style="background-color: var(--bg-primary, #0B0E11); color: var(--text-primary, #E6EAF0); border: 1px solid var(--border-dark, #1F2630);">' +
                '<input type="hidden" step="any" min="0" name="currency_id[]" value="' + data.id + '" class="form-control" required>' +
                '<span class="input-group-text append_currency" style="background-color: var(--bg-elevated, #171C23); color: var(--gold, #D4AF5A); border: 1px solid var(--border-dark, #1F2630);">' + data.currency + '</span>' +
                '<button type="button" class="border-0 font-24 mr-5 ms-3 removedItem text-danger bg-transparent" title="Remove"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M19 6.41L17.59 5L12 10.59L6.41 5L5 6.41L10.59 12L5 17.59L6.41 19L12 13.41L17.59 19L19 17.59L13.41 12L19 6.41Z"/></svg></button>' +
                '</div>';
        });
    ...
    window.addBank = function () {
        return `<div class="multi-bank bg-off-white radius-4 theme-border pb-0 mb-25">
                    <div class="row rg-15">
                        <div class="col-6">
                        <div class="primary-form-group mt-2">
                                    <div class="primary-form-group-wrap">
